...
def homepage(request):
    ...
    klausurThemen = Thema.objects.filter(is_klausur=True)
    if klausurThemen.exists():
        isKlausurSoon = True
    kartenCount = []
    for t in klausurThemen:
        gefundene_karten = Karten.objects.filter(
            karten_thema__link=t.link).exclude(is_published="nein")
        kartenCount.append(len(gefundene_karten))

    return render(request=request,
                  template_name="main/faecher.html",
                  context={"faecher": Fach.objects.all, "neusteThemen": neusteThemen, "klausurThemen": klausurThemen, "kartenCount": kartenCount, "isKlausurSoon": isKlausurSoon})
...
def ThemaErstellen(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, "Ein neuer Beitrag wurde erstellt und wird geprueft: '{}'. Danke fuer die Bemuehungen :)".format(themen_name))
...
def KartenErstellen(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            new_karte = Karten(frage=form.cleaned_data.get("frage"), antwort=form.cleaned_data.get(
                "antwort"), karten_thema=form.cleaned_data.get("karten_thema"), is_published="nein")
            new_karte.save()
...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, "Neuer Account wurde erstellt: {}".format(username))
            login(request, user)
            messages.info(
                request, "Du bist nun eingeloggt als {}".format(username))
...
def erstes_stueck(request, erstes_stueck):
    ...
    if erstes_stueck in themen:
        ...
        gefundene_karten = Karten.objects.filter(
            karten_thema__link=erstes_stueck)
        return render(request,
                      "main/thema.html",
                      {"thema": gefundenes_thema, "themen": gefundene_themen,
                          "KarteiK": gefundene_karten, "aktUser": aktUser, "curSlug": erstes_stueck},
                      )
...
def karten(request, erstes_stueck):
    ...
    if erstes_stueck in themen:
        ...
        return render(request,
                      "main/karten.html",
                      {"Kartei": gefundene_karten, "zurueck": erstes_stueck,
    ...
    if request.user == thema_obj.author:
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                themen_name = form.cleaned_data.get("name")
                messages.success(
                    request, "Beitrag wurde editiert".format(themen_name))
